...
<<<<<<< Updated upstream
  ...
  useEffect(() => {
    (async () => {
      try {
        if (Platform.OS === 'web') { setOcrAvailable(false); return; }
        const mod = await import('expo-text-extractor'); // lazy import to avoid crash
        const isSupported = (mod as any).isSupported as boolean | undefined;
        // some versions export isSupported; if missing, just assume true (dev build should contain the native module)
        setOcrAvailable(isSupported ?? true);
      } catch (e: any) {
